...
@prediction_router.get( "/predict", response_model=PredictionResponse )
async def get_predict( request: PredictionRequest ) -> PredictionResponse:
    return prediction_controller.predict(request)


@prediction_router.get( "/predict/day", response_model=DayPredictionResponse )
...
@prediction_router.get( "/predict/week", response_model=WeekPredictionResponse )
